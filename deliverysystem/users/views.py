...
def editProduct_form_submission(request, id):
    product = Product.objects.get(id = id)
    #print(product)
    print(request.POST.get('name'))
    print(id)
    if request.POST.get('name') and request.POST.get('description') and request.POST.get('price') and request.POST.get(
            'vegan') and request.POST.get('vegetarian'):
        ...
        product.vegan = request.POST.get('vegan')
        product.vegetarian = request.POST.get('vegetarian')
        product.save()
    return render(request, 'users/adminProductEdit.html', {'product': product})


def deleteProduct_form_submission(request, id):
    product = Product.objects.get(id = id)
    rest = product.menu_id
    if request.method == "POST":
        product.delete()
        return redirect("/adminMenu/{}/".format(rest))
    return render(request, 'users/adminViewProduct.html', {'product': product})
